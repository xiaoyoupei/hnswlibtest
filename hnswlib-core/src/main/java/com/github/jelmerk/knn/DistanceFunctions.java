...
import com.github.jelmerk.knn.util.FeatureCompUtil;
...
public final class DistanceFunctions {
     ...
     * ly-add
     */
    static class ByteComp implements DistanceFunction<byte[], Float> {
        ...
        public Float distance(byte[] u, byte[] v) {
            float result = 0;
            result = (float) (pointMulti(u, v) / sqrtMulti(u, v));
            return 1 - result;
        }
    }

    // 点乘法
    private static double pointMulti(byte[] u, byte[] v) {
        double result = 0;
        for (int i = 0; i < u.length; i++) {
            result += (u[i] * v[i]);
        }
    ...
    private static double sqrtMulti(byte[] u, byte[] v) {
        double result = 0;
        result = squares(u, v);
    ...
    // 求平方和
    private static double squares(byte[] u, byte[] v) {
        double result1 = 0;
        double result2 = 0;
        for (int i = 0; i < u.length; i++) {
            result1 += (u[i] * u[i]);
     ...
     * ly-add
     */
    public static final DistanceFunction<byte[], Float> BYTE_COMP = new ByteComp();
