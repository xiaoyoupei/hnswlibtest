...
public class FeatureCompUtil {
    ...
    public static void main(String[] args) throws Exception {
        ...
        byte[] bin1 = base64.decode(f1);
        byte[] bin2 = base64.decode(f2);
        ...
        byte[] raw1 = base64.decode(f1.substring(16));
        byte[] raw2 = base64.decode(f2.substring(16));
    ...
    public List<Double> getDoubleList(byte[] bytes, int offset) {
        ...
        int len = (bytes.length - offset) / 4;
        List<Double> feature = new ArrayList<Double>(len);
        for (int i = 0; i < len; i++) {
    ...
    public List<Float> getFloatList(byte[] bytes, int offset) {
        ...
        int len = (bytes.length - offset) / 4;
        List<Float> feature = new ArrayList<Float>(len);
        for (int i = 0; i < len; i++) {
    ...
    public float Normalize(float score) {
        ...
        for (int i = 1; i < src_points.length; i++) {
            if (score < src_points[i]) {
                result =
                dst_points[i - 1] + (score - src_points[i - 1]) * (dst_points[i] - dst_points[i - 1]) / (src_points[i] - src_points[i - 1]);
                break;
            }
        }
    ...
    public float reversalNormalize(float score) {
        ...
        for (int i = 1; i < (dst_points.length); i++) {
            if (score < dst_points[i]) {
                result =
                src_points[i - 1] + (score - dst_points[i - 1]) * (src_points[i] - src_points[i - 1]) / (dst_points[i] - dst_points[i - 1]);
                break;
            }
        }
        return result;
