 ...
 * @Description：
 */
public class KryoTestItemNew implements ObjectSerializerNew<TestItem>  {
    ...
    public void write(TestItem item, OutputStream out) throws IOException {
//        Kryo kryo = new Kryo();
//        kryo.setReferences(true); //默认值就是 true，添加此行的目的是为了提醒维护者，不要改变这个配置
//
//        //不强制要求注册类（注册行为无法保证多个 JVM 内同一个类的注册编号相同；而且业务系统中大量的 Class 也难以一一注册）
//        kryo.setRegistrationRequired(true); //默认值就是 false，添加此行的目的是为了提醒维护者，不要改变这个配置
//        kryo.register(com.github.jelmerk.knn.hnsw.TestItem.class);
...
//        kryo.setInstantiatorStrategy(new StdInstantiatorStrategy());
    ...
    public TestItem read(InputStream in) throws IOException, ClassNotFoundException {
...
//                .setFallbackInstantiatorStrategy(new StdInstantiatorStrategy());
        ...
        return kryo.readObject(input, TestItem.class);
    }
}
